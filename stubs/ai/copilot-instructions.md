<!-- docs-builder:start -->
# lynkbyte/docs-builder — GitHub Copilot Instructions
...
- `resources/css/themes/` — Theme-specific CSS source files
- `resources/js/themes/` — Theme-specific JS source files
- `resources/views/themes/` — Theme-specific Blade template overrides
- `stubs/` — Scaffold templates for `docs:init`
- `stubs/ai/` — AI tool configuration stubs
...
1. Create theme CSS at `resources/css/themes/{name}.css` (use `resources/css/themes/modern.css` as reference).
2. Create theme JS at `resources/js/themes/{name}.js` (use `resources/js/themes/modern.js` as reference).
3. Add Vite entry points in `vite.config.js` for the new theme's CSS and JS.
4. Create Blade template overrides at `resources/views/themes/{name}/docs/` — only override templates that differ from the default.
5. The service provider auto-detects theme directories via the `theme_name` config key — no registration code needed.
6. Run `npm run build` to compile pre-compiled assets to `dist/themes/`.
7. Ensure all data-attribute contracts are preserved: `data-search-trigger`, `data-theme-toggle`, `data-theme-icon`, `data-copy-code`, `data-mermaid-zoom-in/out/reset/pan/fullscreen`, and standard element IDs.
...
- **Theme architecture** — Themes use view-namespace prepending. When `theme_name` is not `'default'`, the service provider prepends the theme's view directory so Laravel resolves theme views first, falling back to default for any missing files. Theme CSS/JS live in `resources/css/themes/` and `resources/js/themes/`, compiled to `dist/themes/`.
