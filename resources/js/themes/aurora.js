...
/*
|--------------------------------------------------------------------------
| LynkByte DevDocs — Aurora Theme JavaScript
|--------------------------------------------------------------------------
...
|
| Dark-first theme — .dark class is applied by default on <html>.
*/
...
(function () {
    ...
    function performSearch(query) {
        ...
        if (!query || query.length < 2) {
            resultsContainer.innerHTML = '<div class="px-5 py-10 text-center text-sm text-[var(--docs-fg-3)]"><span class="material-symbols-outlined text-2xl mb-2 block opacity-40">travel_explore</span>Type to search documentation...</div>';
            return;
        }
        ...
        if (results.length === 0) {
            resultsContainer.innerHTML = '<div class="px-5 py-10 text-center text-sm text-[var(--docs-fg-3)]"><span class="material-symbols-outlined text-2xl mb-2 block opacity-40">search_off</span>No results found.</div>';
            return;
        }
        ...
        for (const [section, items] of Object.entries(grouped)) {
            ...
            html += `<h3 class="text-[10px] font-bold text-[var(--docs-fg-3)] uppercase tracking-[0.12em] px-3 mb-2 font-mono">${escapeHtml(section)}</h3>`;
            ...
            items.forEach(result => {
                ...
                if (item.description) {
                    html += `<p class="text-[var(--docs-fg-3)] text-xs mt-0.5 truncate">${escapeHtml(item.description)}</p>`;
                }
                html += `</div>`;
                html += `<span class="material-symbols-outlined text-[16px] text-[var(--docs-fg-3)] opacity-0 group-hover:opacity-100 transition-opacity shrink-0">arrow_forward</span>`;
                html += `</a>`;
                index++;
            });
            html += `</div></section>`;
